# lazily modified run-clang-tidy.py
...
def get_tidy_invocation(
    ...
    if use_color is not None:
        if use_color:
            start.append("--color=always")
        else:
            start.append("--color=never")
    if checks:
        start.append(f"-checks={checks}")
    if tmpdir is not None:
        start.append("-export-fixes")
        # Get a temporary file. We immediately close the handle so clangd-tidy can
        # overwrite it.
...
async def run_tidy(
    ...
    """
    Runs clangd-tidy on a single file and returns the result.
    """
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description="Runs clangd-tidy over all files "
        "in a compilation database. Requires "
        "clangd-tidy and clang-apply-replacements in "
    ...
    parser.add_argument(
        "-checks",
        default=None,
        help="Checks filter, when not specified, use clangd-tidy default.",
    )
    ...
    config_group.add_argument(
        ...
        "  -config=\"{Checks: '*', "
        '                       CheckOptions: {x: y}}" '
        "When the value is empty, clangd-tidy will "
    ...
    parser.add_argument(
        ...
        nargs="?",
        const=True,
        help="Use colors in diagnostics, overriding clangd-tidy's"
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Load the specified plugin in clangd-tidy.",
    )
    parser.add_argument(
        "-warnings-as-errors",
        default=None,
    ...
    clangd_tidy_binary = find_binary(args.clangd_tidy_binary, "clangd-tidy", build_path)
